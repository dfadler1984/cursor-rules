...
# Legacy copy of .cursor/scripts/alp-threshold.sh
...
if [[ "$auto" != "0" && "$count" -ge "$threshold" ]]; then
  summary_path="$(docs/projects/assistant-self-improvement/legacy/scripts/alp-aggregate.sh 2>/dev/null || true)"
  mapfile -t oldest_list < <(ls -1 "$LOG_DIR"/log-*.md 2>/dev/null | sort | head -n "$threshold")
  for f in "${oldest_list[@]}"; do
    [[ -f "$f" ]] || continue
    docs/projects/assistant-self-improvement/legacy/scripts/alp-archive.sh mark "$f" >/dev/null 2>&1 || true
  done
  archive_out="$(docs/projects/assistant-self-improvement/legacy/scripts/alp-archive.sh archive 2>/dev/null || true)"
