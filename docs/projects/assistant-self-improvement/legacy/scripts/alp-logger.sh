...
# Legacy copy of .cursor/scripts/alp-logger.sh
...
redact_if_possible() {
  local body="$1"
  if [[ -x "$(dirname "$0")/alp-redaction.sh" ]]; then
    if redacted="$(printf "%s" "$body" | "$(dirname "$0")/alp-redaction.sh" redact 2>/dev/null)"; then
      printf '%s' "$redacted"
      return 0
    fi
  fi
